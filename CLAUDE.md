...
│   │   └── cli/                 # quorum binary CLI commands (init.h, agent_create.h, skills.h)
│   └── tests/
...
- `ConversationEngine` — header-only class in `daemon/conversation.h`. Constructor takes `Database&`, `ConversationConfig&`, `vector<AgentMetadata>&`, `ContextAssembler*`, `project_root`. Methods: `start()`, `on_task_complete()`, `respond()`, `resume()`, `close()`, `recover()`.
...
12. ~~CLI subcommands~~ ✓ (`init`, `converse`, `status`, `resume`, `close`, `agent create`, `agent modify`, `agent list`, `teams`, `skills`, auto-discovery)
...
- ~~**Stale active tasks after crash:**~~ ✓ Fixed — `recover_stale_tasks()` runs on every daemon startup. Finds tasks stuck in `status='active'` from a previous crash/kill/OOM, marks them `failed`, and re-dispatches affected active conversations to leader via `ConversationEngine::recover()`. Prevents the deadlock where sequential dispatch sees `active > 0` and skips dispatch forever.
...
# Add agents with a skill (--skill shorthand for .claude/skills/<name>/SKILL.md)
...
quorum agent modify --name my-dev --skill move-developer  # attach a skill
...
# Scaffold a new agent with a skill (--skill shorthand)
quorum agent create --role doer --name my-doer --target-dir . --skill move-developer
