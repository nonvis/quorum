...
- **Capture / write task** — the task tells you to *emit, produce, refresh, or
  ...
  vault (`knowledge/ref-architecture-map.md`) and HANDOFF — **once, now.** In a
...
- **Discussion-only task** — the task asks you to *explore, weigh in on, or
  ...
  Contribute your interconnection/architecture analysis as plain reasoning + a
  SUMMARY, then **end your turn with NO HANDOFF** — the daemon returns the ball
  to the leader (do NOT `HANDOFF to: leader`; just omit the HANDOFF block).
  **Emit NO `VAULT_UPDATE`** (nothing to capture yet).
...
re-scan. Update `ref-architecture-map.md` in place as always.
